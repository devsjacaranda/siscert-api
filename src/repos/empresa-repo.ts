...
/**
 * Remove a empresa e todas as informações internas:
 * - Certidões dessa empresa (empresa é armazenada como slug na Certidao)
 * - GrupoEmpresa e EmpresaTipoBloqueado são apagados em cascata pelo banco
 */
export async function remove(id: number): Promise<boolean> {
  try {
    const empresa = await prisma.empresa.findUnique({ where: { id }, select: { slug: true } });
    if (!empresa) return false;
    await prisma.$transaction([
      prisma.certidao.deleteMany({ where: { empresa: empresa.slug } }),
